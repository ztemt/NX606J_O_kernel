...
int fb_switch_notifier_callback(struct notifier_block *self,
...
static void himax_fb_switch_register(struct work_struct *work){
    int ret = 0;
    struct himax_ts_data *ts = container_of(work, struct himax_ts_data,
                                            work_att.work);
    I(" %s in\n", __func__);
    ts->fb_notif.notifier_call = fb_switch_notifier_callback;
    ret = msm_drm_switch_register_client(&ts->fb_notif);
    if (ret < 0) {
        E("%s: Failed to register fb_notifier client  :  %d\n", __func__, ret);
    }
}
...
FW_force_upgrade:
    ...
    INIT_DELAYED_WORK(&ts->work_att, himax_fb_switch_register);
