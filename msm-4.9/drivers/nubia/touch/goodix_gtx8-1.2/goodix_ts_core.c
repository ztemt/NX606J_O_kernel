...
static void  __do_register_ext_module(struct work_struct *work)
{
	...
	ts_err("__do_register_ext_module IN, goodix_modules.core_exit:%d", goodix_modules.core_exit);
...
*/
	msleep(500);
	ts_err("goodix_register_ext_module IN");

	INIT_WORK(&module->work, __do_register_ext_module);
	schedule_work(&module->work);

	ts_err("goodix_register_ext_module OUT");
...
int goodix_unregister_ext_module(struct goodix_ext_module *module)
{
	...
	ts_info("Moudle [%s] unregistered",
			module->name ? module->name : " ");
	if(!strncmp(module->name, "goodix-fwu", strlen("goodix-fwu"))){
		...
		struct goodix_touch_data *touch_data)
{
	...
	for (i = 0; i < ts_bdata->panel_max_id * 2; i++) {
		if (touch_num && i == id) { /* this is a valid touch down event */
			...
			if (i == 0)
				ts_info("***********************************report down:%d, %04x%04x", i, coords->x, coords->y);
			if (i == 1)
				ts_info("********************************************report down:%d, %04x%04x", i, coords->x, coords->y);
			if (i == 2)
				ts_info("*****************************************************report down:%d, %04x%04x", i, coords->x, coords->y);
...
static void goodix_ts_esd_work(struct work_struct *work)
{
	...
	if (r < 0) {
		...
		r = hw_ops->write_trans(core->ts_dev,
				...
				&data, 1);
		if (r < 0)
			ts_err("esd reset, init watch dog FAILED, i2c write ERROR");
...
int goodix_ts_esd_init(struct goodix_ts_core *core)
{
	...
	if (core->ts_dev->board_data->esd_default_on &&
		...
		r = dev->hw_ops->write_trans(core->ts_dev,
				...
				&data, 1);
		if (r < 0)
			ts_err("ESD init ERROR, i2c write failed");
...
int goodix_ts_fb_switch_notifier_callback(struct notifier_block *self,
                         unsigned long event, void *data)
{
	struct goodix_ts_core *core_data =
		container_of(self, struct goodix_ts_core, fb_notifier);
    ...
    if (fb_event && fb_event->data && core_data)
    {
        ...
        else if(event == MSM_DRM_SWITCH_EARLY_EVENT_BLANK){
            blank = fb_event->data;
            switch (*blank)
            {
                case MSM_DRM_SLAVE_BLANK_UNBLANK:
