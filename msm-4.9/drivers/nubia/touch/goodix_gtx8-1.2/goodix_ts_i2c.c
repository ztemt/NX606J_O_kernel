...
static int goodix_set_i2c_doze_mode(struct goodix_ts_device *dev, int enable)
{
	...
	if (enable) {
		...
		if (dev->doze_mode_set_count == 0) {
			w_data = 0xCC;
			for (i = 0; i < 3; i++) {
				result = goodix_i2c_write_trans(dev, 0x30F0, &w_data, 1);
				if (!result) {
					result = 0;
					goto exit;
				}
		...
		if (dev->doze_mode_set_count == 1) {
			for (i = 0;i < 3;i++) {
				w_data = 0xAA;
				goodix_i2c_write_trans(dev, 0x30F0, &w_data, 1);
				usleep_range(1000, 1100);
				for (j = 0;j < 3;j++) {
					goodix_i2c_read_trans(dev, 0x3100, &r_data, 1);
					if (0xBB == r_data) {
						result = 0;
						goto exit;
					} else {
						/*ts_err("0x3100 not equal 0xbb, retry:%d", j);*/
						usleep_range(10000, 10100);
					}
				}

				/*ts_err("i2c doze mode disable failed, retry:%d", i);*/
			}
			if (i >= 3)
				ts_err("doze mode disable FAILED");
		} else {
			/*ts_info("doze mode has been disabled, so skip this step");*/
			result = 0;
			goto exit;
		}
	}
		...
		u8 *pre_buf, u32 pre_buf_len)
{
	...
	for (i = 0; i < touch_num - 1; i++) {
		...
		coords->id = buffer[i * BYTES_PER_COORD + 2];
...
exit_clean_sta:
	...
	buffer[0] = 0x00;
	buffer[1] = 0x00;
	goodix_i2c_write_trans(dev, dev->reg.coor, buffer, 2);/*TS_REG_COORDS_BASE*/
	return r;
}
...
static int goodix_esd_check(struct goodix_ts_device *dev)
{
	...
	if (r < 0 || (data == 0xaa)) {
		ts_info("esd occur, r:%d, data:0x%02x", r, data);
		r = -EINVAL;
	}
