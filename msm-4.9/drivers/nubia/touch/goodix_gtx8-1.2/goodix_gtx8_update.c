		...
		const struct firmware_info *fw_info)
{
	...
	if (fw_ver.valid) {
		if (memcmp(fw_ver.pid, fw_info->fw_pid, dev->reg.pid_len)) {
			ts_err("fw_info->fw_pid = %s, fw_ver.pid = %s,dev->reg.pid_len = %d\n", fw_info->fw_pid, fw_ver.pid, dev->reg.pid_len);
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	ts_dev->hw_ops->write(ts_dev, 0xBDE6, temp_buf, 8);
	ts_dev->hw_ops->write(ts_dev, 0xBE96, temp_buf, 8);
	ts_dev->hw_ops->write(ts_dev, 0x452C, temp_buf, 8);
	...
	*/
	...
	do {
		...
		if (r < 0) {
			...
			reg_val[0] = 0x01;
			goodix_reg_write(ts_dev, 0x2044, reg_val, 1);
			msleep(20);
		} else {
			break;
		}
				 ...
				 u8 *pkt, u32 len)
{
	...
	reg_val[0] = 0;
	/* clear flash flag 0X8022 */
	r = goodix_reg_write_confirm(dev, HW_REG_FLASH_FLAG, reg_val, 1);
	if (r < 0) {
	...
	ts_err("Wait for flash end timeout");
	return -EAGAIN;
}
				...
				struct fw_update_ctrl *fwu_ctrl)
{
	...
	reg_val[0] = 0x01;
	r = goodix_reg_write(ts_dev, 0x2044, reg_val, 1);

	if (r < 0)
		ts_err("Failed write 0x2044");
